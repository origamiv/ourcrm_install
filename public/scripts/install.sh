...
EOF
  ...
  # Best-effort: update desktop database for user
  sudo -u "$user" update-desktop-database "$apps_dir" >/dev/null 2>&1 || true
}

# Clears Cinnamon menu caches for user (so new entries appear after relogin)
...
# + required packages for headless VNC Cinnamon
# ------------------------------------------------------------
...
# Run Cinnamon inside a D-Bus session (required on headless VNC)
...
# TigerVNC systemd service (port 5901) - oneshot to avoid PIDFile issues
...
# ------------------------------------------------------------
# snap + apps (robust) + JetBrains IDEs
...
# Snap desktop integration (THIS is a snap, not an apt package)
...
# This fixes "not visible in menu" for snaps in Cinnamon/VNC.
# ------------------------------------------------------------
echo "$LOG Creating Cinnamon menu shortcuts for ${MAIN_USER}..."

# Always create, even if snap didn't export /var/lib/snapd/desktop/applications
# (Cinnamon reliably shows ~/.local/share/applications/*.desktop)
...
# Chrome is deb; icon/desktop usually exists, but create a user entry anyway
...
# Refresh desktop databases (system + user)
...
# Clear Cinnamon menu caches so new items appear after relogin
