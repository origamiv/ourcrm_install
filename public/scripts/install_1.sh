...
# Added required packages for headless VNC Cinnamon: dbus-x11 dbus-user-session xauth
# ------------------------------------------------------------
...
# Key fixes:
# - xstartup uses dbus-launch --exit-with-session cinnamon-session
# - systemd unit uses Type=oneshot + RemainAfterExit (no PIDFile hostname mismatch/timeouts)
# - VNC uses SecurityTypes VncAuth (xrdp/libvnc compatible)
# ------------------------------------------------------------
...
# Fixes:
# - start snapd via snapd.socket
# - wait for seed.loaded
# - retries
# - telegram-desktop WITHOUT --classic
# ------------------------------------------------------------
...
snap list 2>/dev/null | egrep -i 'phpstorm|pycharm|telegram-desktop|chromium|firefox|core' || true
